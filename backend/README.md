...
with migration `0007` and the seller route tree `src/web/routes/seller.ts`. Founder-controlled
...
| Throttling (`AUTH-204`)                         | `throttle.ts`, `auth.reserve_sign_in_attempt`          | Attempts are counted under a row lock before any credential work, per hashed account and per hashed client. Free allowance 3 (account) and 10 (client); then `base × 2^(n−free−1)` seconds capped at 60 s (account) and 900 s (client); refusals answer `429` with `Retry-After` and do not extend the lock; counters decay after an hour; success clears the account counter only |
...
| Data access (`OPS-716`)                         | migration `0007`                                       | Schema `auth` is owned by `app_migrator`; `app_runtime` holds no table privilege and executes ten `SECURITY DEFINER` keyhole functions with a pinned `search_path`, each taking the one identifier it needs. The runtime role cannot create an account                                                                                                                             |
| Audit (`AUTH-217`)                              | `ai/POLICY_AND_AUTHORIZATION.md` §12                   | `SELLER_SIGN_IN_SUCCEEDED`, `SELLER_SESSION_ROTATED`, `SELLER_SIGNED_OUT`, `SELLER_SESSIONS_REVOKED` in the seller's audit trail; `SELLER_SIGN_IN_FAILED`, `SELLER_SIGN_IN_THROTTLED` in the pre-authentication ledger `auth.sign_in_event`, hashed identifiers only                                                                                                               |
...
`test/unit/seller-auth-*.test.ts`, `test/integration/seller-auth*.test.ts` (the last of which is
the `AUTH-203` timing-distribution gate and prints its observations), plus the auth-schema
checks in `migrations.test.ts` and the route inventory in `web.test.ts`.
