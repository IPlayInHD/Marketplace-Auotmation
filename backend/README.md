...
with migrations `0007` and `0008` and the seller route tree `src/web/routes/seller.ts`. Founder-controlled
...
| Data access (`OPS-716`)                         | migrations `0007`, `0008`                                                                         | Schema `auth` is owned by `app_migrator`; `app_runtime` holds no table privilege and executes ten `SECURITY DEFINER` keyhole functions with a pinned `search_path`, each taking the one identifier it needs. The runtime role cannot create an account                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
