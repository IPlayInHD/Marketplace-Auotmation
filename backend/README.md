...
Three authenticated routes expose three existing commands of Module 3 (`src/modules/listings/`)
without changing their semantics, registered into the seller tree by
...
| ------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- | ----------------- |
| `POST /seller/listings`                          | seller · listing · create · seller-session · tenant from session · consequential · `Idempotency-Key` required · `LISTING_CREATED`                        | `{ "inventoryItemId": uuid }`, the only field `listing.create` accepts; the item must belong to the tenant (`LIST-100`)                      | `201 { listing }` |
| `GET /seller/listings/:listingId`                | seller · listing · read · seller-session · tenant from session · read-only · no key · no event                                                           | none                                                                                                                                         | `200 { listing }` |
...
`400 idempotency_key_required`, `404 not_found` (a listing or item that is not this tenant's or
does not exist), `409 invalid_state` (a price change outside `DRAFT`, or a second listing for an
item that already carries a live one), `409 stale_row_version` (`OPS-738`),
...
Not here, by design: a route that creates an inventory item (the tests create items through the
domain command), listing enumeration, facts, content, publication, closure, photos, buyer
surfaces.
