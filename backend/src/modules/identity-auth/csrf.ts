...
// value in a header a cross-site page cannot set. Sign-in has no session yet, so it relies on
// the origin and Fetch Metadata checks alone (TM-41).
...
export function verifyAntiForgery(headers: IncomingHttpHeaders, sessionToken: string): boolean {
  ...
  if (presented === undefined || presented.length > 128) return false;
