...
| `RISK-22` | Model provider availability, pricing or terms change adversely | Dependency / financial | Medium | High | **High** | No provider is a requirement anywhere in the documentation (`D-08`, `LIST-067`; D-17 keeps the provider open as `Q-10`); secondary provider in the failure posture (`ARCHITECTURE.md` §9); tiers are named roles (`cheap`, `mid`, `premium`), not products; evals make a provider swap measurable rather than a leap of faith (`EVAL_STRATEGY.md`); cost model recomputed on any rate change (`OPS-441`) | Engineering | Provider pricing announcements; latency or error-rate drift; eval scoreboard movement after a provider-side model update | `MONITORED` |
...
| `RISK-24` | The job queue, pg-boss, is a single-maintainer open-source dependency (its own README says so): it could stall, break on a Node.js or PostgreSQL upgrade, or carry an unpatched vulnerability (D-17) | Dependency / engineering | Medium | Medium | **Medium** | The pg-boss operating rule, `engineering/OPERATIONS.md` §2.1 (`OPS-522` to `OPS-524`): exact version pin and committed lockfile with no automatic upgrade (`SEC-380`); schema migrations only through the migration/owner role, runtime started with migration disabled, runtime roles holding no DDL or ownership (`OPS-716`); queue topology changes treated as migrations; changelog and migration review, the complete foundation spike suite, backup and recovery planning and a tested rollback or forward-recovery plan before every upgrade; dependency and maintainer-health review at least monthly with this register (`BIZ-204`) and before every upgrade; named reconsideration triggers (`OPS-524`); queue access behind one internal interface so a replacement PostgreSQL-backed queue is a bounded change; dependency vulnerability scanning (`SEC-382`). The D-17 acceptance spike (commit `08d2d86`) passed every proof on pg-boss 12.29.0 | Engineering | Maintainer inactivity or unmerged security fixes; CI scan findings against the pinned version; failures after a Node.js or PostgreSQL upgrade; the foundation spike suite failing on a candidate upgrade; any `OPS-524` trigger | `OPEN` - operating rule in force; reviewed monthly |
...
| **Passes if** | Link-open rate above ~40% across at least 100 real buyer contacts, and code-entry completion at 80% or higher of opens - the clean pass on the reconciled bands of `planning/MVP_ROADMAP.md` Slice 0 (D-17, C-09). |
| **Revise and re-run if** | Code-entry completion is between 50% and 79% of opens. Revise the entry flow - pre-fill, URL shape, preview content, disclosure wording - and run the validation again. This is not a pass. |
| **Fails if** | Link-open rate misses ~40%, or code-entry completion falls below 50% of opens. Stop, or fundamentally rework the concept, **not** build the product and hope. These bands describe how the data is read; they are not evidence, and the test must actually run. |
| **Owner** | Product |
