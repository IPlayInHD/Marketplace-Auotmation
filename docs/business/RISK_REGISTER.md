...
| `RISK-24` | The provisional job queue, pg-boss, is a single-maintainer open-source dependency: it could stall, break on a Node.js or PostgreSQL upgrade, or carry an unpatched vulnerability (D-17) | Dependency / engineering | Medium | Medium | **Medium** | Pinned dependency version and lockfile-only builds (`SEC-380`); installation and upgrades only through the migration role or a controlled CLI step, never the runtime role (D-17, `OPS-716`); queue access behind one internal interface so a replacement PostgreSQL-backed queue is a bounded change; acceptance of D-17 is conditional on a spike proving transactional enqueueing, retry behaviour, redelivery and restricted-role operation; dependency vulnerability scanning (`SEC-382`) | Engineering | Maintainer inactivity or unmerged security fixes; CI scan findings against the pinned version; failures after a Node.js or PostgreSQL upgrade; the spike failing any of its four proofs | `OPEN` - spike pending |
