...
| TM-37 | Partial failure leaves an inconsistent deal | Crash between listing transition and approval status write | A listing in `PENDING_SALE` with no executed approval — the stuck deal of `OPERATIONS.md` §9.6 | One transaction for the assertions and transitions (`AUTH-244`); approval row written before execution (`AUTH-243`) | Reconciliation job (`OPS-728`) with page (`OPS-595`) | Low | Integration: crash injected between write and execute |
...
| SEC-333 | The instance metadata service, if one exists in the chosen infrastructure, is unreachable from the application process or requires a header the application never sends. Confirmed at stack selection (`D-08`). |
