...
This directory holds the reproducible evidence behind decision **D-19 (Proposed)** in
`docs/decisions/DECISION_LOG.md`, the bounded evaluation of the seller-authentication approach
that open question `Q-12` asks for. It implements no product behaviour: no sign-up page, no
seller route tree, no production migration, no buyer authentication, no email. Delete it once
D-19 is accepted, revised or rejected.

Everything lives under `spikes/authentication/`. Nothing here is imported by application code.
The test harness *reads* `backend/src/db/migrations/*.sql` from disk, unmodified, to build the
real schema inside a throw-away container; it never writes to `backend/`.

## What is proved

The leading candidate is a first-party module built from focused primitives already in the
...
Remove `spikes/authentication/`. Nothing else references it.
